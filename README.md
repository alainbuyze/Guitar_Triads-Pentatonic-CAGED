...
4. **The full chord** fills in the three strings outside the selected triad by borrowing notes from any *other* string-set's triad that lands in the exact same box (same shape, same fret window) — rather than independently guessing the nearest chord tone. Since every triad in a box is already cross-checked against the others this way, the full chord always agrees with what the app's own triads say, matching the real open/barre chord fingering (e.g. a C-shape always doubles the 3rd, never the 5th, on the string outside the box's own 3-string window). Only a string with no matching triad anywhere (a string a real open chord would leave muted) falls back to a nearest-chord-tone guess.
